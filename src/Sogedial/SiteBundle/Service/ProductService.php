...
use Sogedial\SiteBundle\Service\ExportService;
...
class ProductService
{
    ...
    public function __construct(MultiSiteService $ms, EntityManager $em, SimpleMySQLService $sql, UserInfo $ui, ExportService $export)
    {
        $this->ms = $ms;
        $this->em = $em;
        $this->sql = $sql;
    ...
    public function getPromosQueryElements($code_enseigne = false, $code_client= false)
    {
        if($code_enseigne !== false && $code_client !== false){
            $codeClientString = '"'. $code_client .'"';
            $codeEnseigneString = '"'. $code_enseigne .'"';
        }
        else{
...
//        $productsByClientInPromotion, $productsByEnseInPromotion, $promos,
        $unitedPromos,
        $code_enseigne_from_admin = false, 
        $code_tarification_from_admin= false)
    {
        $result = array();

        if($code_enseigne_from_admin !== false && $code_tarification_from_admin !== false){
            $code_tarification = $code_tarification_from_admin;
            $code_enseigne = $code_enseigne_from_admin;
        }
        else{
        ...
        if (isset($promo)) {
            $result['priceArray'][1] = $promo['prix_ht'];         // 1 = quantité minimale (pseudo-palier)
            ...
            if($this->ms->hasFeature('tarifs-tarification')){
                ...
                $actualPrice = $this->calculTarifMarge($product, $code_enseigne);
            } else if($this->ms->hasFeature('tarifs-degressifs')) {
                foreach ($tarifs as $tarif) {
                    if ($tarif->getEnseigne()->getCode() === $code_enseigne) {            // dans ce cas, les tarifs *doivent* avoir un code enseigne
                        $actualPrice = $tarif->getPrixHt();
                        break;
            ...
            if ($actualPrice !== null) {
                ...
                if( $decreasingPrices ) {
                    ...
                    $result['priceArray'] = array($step =>  $actualPrice);
                } else {
                    $result['priceType'] = '81TS';
     ...
     * @param $user
     * @param $codeSociete
     * @param bool $code_enseigne_from_admin
     * @param bool $code_tarification_from_admin
     * @return array
     */
    public function getActualProductPriceAndStockForConsole ($product, $unitedPromos, $user, $codeSociete, $code_enseigne_from_admin = false, $code_tarification_from_admin= false)
    {
        $result = array();
        ...
        if( null === $code_tarification && null === $code_enseigne) {
            $client = $this->em->getRepository('SogedialSiteBundle:Client')->findOneBy(array("meta" => $user->getMeta()->getCode(), 'entreprise' => $codeSociete));

            if($client->getTarification() !== null && $client->getTarification() instanceof Tarification) {
                $code_tarification = $client->getTarification()->getCode();
            }

            $code_enseigne = $client->getEnseigne()->getCode();
        }

        if( null === $code_enseigne ) {
            $client = $this->em->getRepository('SogedialSiteBundle:Client')->findOneBy(array("code" => sprintf('%s-%s', $codeSociete, $user->getUsername())));
            $code_enseigne = $client->getEnseigne()->getCode();
        }
    ...
    public function calculTarifMarge($produit, $code_enseigne)
    {
        $clientCodeMarge = $this->export->getMargeFromCodeEnseigne($code_enseigne);

        $marque = $this->em->getRepository('SogedialSiteBundle:Marque')->findOneBy(array("code" => $produit->getMarque()->getCode()));

        $price = 99999999.0;

        if($produit->getDepartement() == '401-5'){
            $price = $produit->getPrixPreste() * $clientCodeMarge[2];
        }
        else{
            if (isset($marque) && $marque->isIsMdd()) {
                $price = $produit->getPrixPreste() * $clientCodeMarge[1];
            } else {
                $price = $produit->getPrixPreste() * $clientCodeMarge[0];
            }
        }
    ...
    {
        return array(
        "fields" => "p.denomination_produit_base as text, p.code_produit as code, p.ean13_produit as ean13, m.libelle as marque, p.pre_commande as preCommande",
        "joins" => "LEFT JOIN marque m ON m.code_marque=p.code_marque"      // joins necessary for obtaining the fields above
        );
    }

    public function getCountEnrichQuery($code_enseigne, $code_tarification, $code_assortiment, $from, $where)
    {
        $productDetailFields = $this->getProductDetailFields();

        $req = "SELECT COUNT(*) as search_count
            FROM ".$from."
            ...
            LEFT JOIN client_produit_moq cpm ON cpm.code_produit = p.code_produit AND cpm.code_client = '".$this->ui->code_client."'
        ...
        if($this->ui->pre_commande !== NULL){
            $req .= "AND p.pre_commande = 1";
            $entreprisePreCommande = $this->em->getRepository('SogedialSiteBundle:Entreprise')->findOneBy(array("code" => $this->ui->entreprise_courante));
            $raisonSocialWords = explode(' ', $entreprisePreCommande->getRaisonSociale());
            $assortimentPreCommandeLabel = array_pop($raisonSocialWords);
            if($assortimentPreCommandeLabel === 'AVION'){
                $req .= " AND p.marketing_code LIKE 'AVION%'";
            } else {
                $req .= " AND p.marketing_code NOT LIKE 'AVION%'";
            }
        } else {
            $req .= " AND p.pre_commande <> 1";
        }

        $req .= " AND p.actif = 1 ".(($where==="")?"":("AND (".$where.")")).
    ...
    public function getEnrichQuery($code_enseigne, $code_tarification, $code_assortiment, $from, $where, $extraFields)
    {
        ...
        $req = "SELECT ".($productDetailFields['fields']).(($extraFields==="") ? "" : (", ".$extraFields)).", IFNULL(cpm.moq_quantite, 0) as moq, t.prix_ht as init_price
            FROM ".$from."
            ".($productDetailFields['joins'])."
            LEFT JOIN famille f ON f.code_famille = p.code_famille
            INNER JOIN assortiment ass ON ass.code_produit=p.code_produit
